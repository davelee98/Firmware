...
# Emitted by od_emit() on the first line that gets through after a drop.
DROP_NOTICE = "[od_log] dropped"
...
def main() -> int:
    ...
    ap.add_argument("--expect-drop-notice", action="store_true",
                    help="also require the '[od_log] dropped N line(s)' report. Off by "
    ...
    if args.expect_drop_notice and DROP_NOTICE not in text:
        print(f"[test] FAIL: device survived, but no {DROP_NOTICE!r} line was seen.")
